...
- (void)setModel:(TXMallUAVModel *)model{
    _model = model;
    TTLog(@"model.sectionType -- %ld",model.sectionType);
...
//    tools.dynamicModel = self.dataArray[indexPath.row];
    ...
    model.sectionType = self.model.sectionType;
    tools.model = model;
    return tools;
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    if (self.model.sectionType==2) {
        return CGSizeMake(190, 190);
    }else if (self.model.sectionType==3) {
        return CGSizeMake(190, 160);
    }else if (self.model.sectionType==4) {

...
typedef enum : NSUInteger {
    LZOptionSelectViewTypeCustom,//默认风格
    LZOptionSelectViewTypeArrow,//带箭头的下拉框
} LZOptionSelectViewType;


///弹出方向,左右上下的区别，只有在上下或者左右都显示不下时，优先考虑的方向
typedef enum : NSUInteger {
    LZOptionSelectViewBottom,//下
    LZOptionSelectViewTop,//上
    LZOptionSelectViewLeft,//左
    LZOptionSelectViewRight//右
} LZOptionSelectViewDirection;


typedef void(^ActionBack)(NSIndexPath*);

@interface LZOptionSelectView : UITableView
...
@property (nonatomic, copy) NSInteger(^rowNumber)(void) ;
@property (nonatomic, copy) float(^optionCellHeight)(void);
#pragma mark - 事件回调
///删除回调
...
/** 风格 */
@property (nonatomic, assign) LZOptionSelectViewType optionType;

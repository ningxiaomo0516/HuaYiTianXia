...
@implementation TTHUDManager

/// 隐藏蒙版默认时间
static const NSTimeInterval kTTHUDHideTimeInterval = 1.0f;
/// 提示框文字大小
static CGFloat kTTHUDDefaultFontSize = 13.0f;
static NSTimer * kTTHUDkHideHUDTimer;
...
+ (void)hideDelay:(NSTimeInterval)delaySeconds{
    [kTTHUDkHideHUDTimer invalidate];
    kTTHUDkHideHUDTimer = [NSTimer xw_timerTimeInterval:delaySeconds block:^{
        [TTHUDManager hide];
    } repeats:NO];
}
...
+ (void)showTipHUD:(NSString *)message {
    [self p_showTipMessage:message isLineFeed:NO isWindow:YES timer:kTTHUDHideTimeInterval];
}
...
+ (void)showTipHUD:(NSString *)message isLineFeed:(BOOL)isLineFeed {
    [self p_showTipMessage:message isLineFeed:isLineFeed isWindow:YES timer:kTTHUDHideTimeInterval];
}

/// 在window上显示文本提示框
+ (void)showTipHUDInView:(NSString *)message isLineFeed:(BOOL)isLineFeed {
    [self p_showTipMessage:message isLineFeed:isLineFeed isWindow:NO timer:kTTHUDHideTimeInterval];
}
...
+ (void)showCustomTipHUD:(NSString *)message isLineFeed:(BOOL)isLineFeed backgroundColor:(UIColor *)backgroundColor textColor:(UIColor *)textColor textFont:(UIFont *)textFont margin:(CGFloat)margin offset:(CGPoint)offset isWindow:(BOOL)isWindow {
    [self p_showCustomTipMessage:message isLineFeed:isLineFeed isWindow:isWindow backgroundColor:backgroundColor textColor:textColor textFont:textFont margin:margin offset:offset timer:kTTHUDHideTimeInterval];
}
...
/// 正确提示
+ (void)showSuccessHUD {
    [self p_showCustomIcon:@"right" message:@"" isWindow:YES timer:kTTHUDHideTimeInterval];
}

/// 有文本正确提示
+ (void)showSuccessTipHUD:(NSString *)message {
    [self p_showCustomIcon:@"right" message:message isWindow:YES timer:kTTHUDHideTimeInterval];
}

/// 在view展示有文本正确提示
+ (void)showSuccessTipHUDInView:(NSString *)message {
    [self p_showCustomIcon:@"right" message:message isWindow:NO timer:kTTHUDHideTimeInterval];
}

/// 错误提示
+ (void)showErrorHUD {
    [self p_showCustomIcon:@"error" message:@"" isWindow:YES timer:kTTHUDHideTimeInterval];
}

/// 有文本错误提示
+ (void)showErrorTipHUD:(NSString *)message {
    [self p_showCustomIcon:@"error" message:message isWindow:YES timer:kTTHUDHideTimeInterval];
}

/// 在view有文本错误提示
+ (void)showErrorTipHUDInView:(NSString *)message {
    [self p_showCustomIcon:@"error" message:message isWindow:NO timer:kTTHUDHideTimeInterval];
}

/// 信息提示
+ (void)showInfoTipHUD:(NSString *)message {
    [self p_showCustomIcon:@"info" message:message isWindow:YES timer:kTTHUDHideTimeInterval];
}

/// 在view信息提示
+ (void)showInfoTipHUDInView:(NSString *)message {
    [self p_showCustomIcon:@"info" message:message isWindow:NO timer:kTTHUDHideTimeInterval];
}

/// 警告提示
+ (void)showWarningTipHUD:(NSString *)message {
    [self p_showCustomIcon:@"tip" message:message isWindow:YES timer:kTTHUDHideTimeInterval];
}

/// 在view警告提示
+ (void)showWarningTipHUDInView:(NSString *)message {
    [self p_showCustomIcon:@"tip" message:message isWindow:NO timer:kTTHUDHideTimeInterval];
}

#pragma mark - 自定义图片
/// 展示自定义图片 - 图片需要导入 'TTHUDImages.bundle' 包中
...
/// 在view上展示自定义图片 - 图片需要导入 'TTHUDImages.bundle' 包中
...
/// 展示自定义图片 - 图片需要导入 'TTHUDImages.bundle' 包中 - 不自动移除
...
/// 在view上展示自定义图片 - 图片需要导入 'TTHUDImages.bundle' 包中 - 不自动移除
...
+ (void)p_showCustomTipMessage:(NSString*)message isLineFeed:(BOOL)isLineFeed isWindow:(BOOL)isWindow backgroundColor:(UIColor *)backgroundColor textColor:(UIColor *)textColor textFont:(UIFont *)textFont margin:(CGFloat)margin offset:(CGPoint)offset timer:(NSTimeInterval)aTimer {
    ...
    hud.detailsLabel.font = hud.label.font = textFont?: [UIFont systemFontOfSize:kTTHUDDefaultFontSize];
...
+ (void)p_showCustomIcon:(NSString *)iconName message:(NSString *)message isWindow:(BOOL)isWindow timer:(NSTimeInterval)aTimer {
    ...
    NSString *normalImgName = [NSString stringWithFormat:@"TTHUDManager_%@@2x.png", iconName];
...
+ (MBProgressHUD *)p_createMBProgressHUDviewWithMessage:(NSString*)message isWindiw:(BOOL)isWindow {
    ...
    hud.detailsLabel.font = hud.label.font = [UIFont systemFontOfSize:kTTHUDDefaultFontSize];
...
/// 获取当前屏幕显示的viewcontroller
+ (UIViewController *)p_getCurrentUIVCS {

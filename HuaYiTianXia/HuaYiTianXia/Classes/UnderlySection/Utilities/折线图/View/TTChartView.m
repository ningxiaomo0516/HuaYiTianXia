...
@interface TTChartView ()<UIScrollViewDelegate>
{
...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(NSArray<NSString *> *)dataSource countFoyY:(NSInteger)countY {
    if (self = [super initWithFrame:frame]) {
        ...
        _lineColor = [UIColor colorWithRed:127/255.0 green:5/255.0 blue:5/255.0 alpha:1];
        _duration = 0.0f;
        _bg_color = kWhiteColor;
...
- (instancetype)initWithFrame:(CGRect)frame countFoyY:(NSInteger)countY {
    return [self initWithFrame:frame dataSource:@[] countFoyY:countY];
}
...
//            btn.hidden = NO;
        ...
        if (_pointImage == nil) {
            _pointImage = [UIImage imageNamed:@"btn_circle"];
        }
        
        if (_selectedPointImage == nil) {
            _selectedPointImage = [UIImage imageNamed:@"btn_point"];
        }
        ...
        detailLabel.font = kFontSizeMedium14;
        NSString *str = arr[i];
        CGSize textSize = [str boundingRectWithSize:CGSizeMake(200, 100) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:kFontSizeMedium15} context:nil].size;;
        [detailLabel setFrame:CGRectMake((_xMargin)*i-_xMargin*0.7-textSize.width/2+btnW/2, height *(1 - percent)-30 , textSize.width+4, textSize.height-2)];
        detailLabel.text = str;
        detailLabel.backgroundColor = self.lineColor;
...
- (void)addLeftViews{
    ...
    for (int i = 0;i< _dataArrOfY.count ;i++ ) {
        ...
        leftLabel.font = kFontSizeMedium10;
        leftLabel.textColor = kTextColor153;
...
- (void)addBottomViews {
    ...
    for (int i = 0;i< _dataArrOfX.count ;i++ ) {
        ...
        if (i == _dataArrOfX.count-1) {
            leftLabel.textColor = kThemeColorHex;
        }
...
- (void)pointBtnAction:(UIButton *)sender{
    ...
    for (int i = 0; i<self.dataArrOfX.count; i++) {
        ...
        label.textColor = kTextColor153;
    }
    UILabel *label = [self viewWithTag:1000+sender.tag-1];
    label.textColor = kThemeColorHex;
...
- (UILabel *)titleLForY {
    if (!_titleLForY) {
        ...
        _titleLForY.font = kFontSizeMedium10;
        _titleLForY.textAlignment = kCTTextAlignmentCenter;
        _titleLForY.textColor = kTextColor153;
    }
    return _titleLForY;
}
...
- (UILabel *)titleLForX {
    if (!_titleLForX) {
        ...
        _titleLForX.font = kFontSizeMedium10;
        _titleLForX.textAlignment = kCTTextAlignmentRight;
        _titleLForX.textColor = kTextColor153;
    }
    return _titleLForX;
}

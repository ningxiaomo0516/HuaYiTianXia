...
@interface SCVerCodeInputView ()<UITextViewDelegate>
...
@end
@implementation SCVerCodeInputView
...
-(void)sc_verCodeViewWithMaxLenght{
    ...
    if (self.contairView) {
        [self.contairView removeFromSuperview];
    }
    [self addSubview:self.contairView];
    [self.contairView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(self.mas_height);
        make.centerX.equalTo(self);
        make.centerY.equalTo(self);
    }];
    [self.contairView addSubview:self.textView];
    //添加textView
    ...
    for (int i=0; i<self.maxLenght; i++) {
        ...
        [self.contairView addSubview:subView];
...
- (UIView *)contairView{
    if (!_contairView) {
        _contairView = [UIView lz_viewWithColor:kClearColor];
    }
    return _contairView;
}

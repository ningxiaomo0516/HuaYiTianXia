...
+ (NSString *)formatHanyuPinyinWithNSString:(NSString *)pinyinStr
  ...
  if (ToneTypeWithoutTone == [outputFormat toneType]) {
     ...
     pinyinStr = [LZPinyinFormatter convertToneNumber2ToneMarkWithNSString:pinyinStr];
  }
...
+ (NSString *)convertToneNumber2ToneMarkWithNSString:(NSString *)pinyinStr {
  ...
  if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]?"]) {
    ...
    if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]"]) {
        int tuneNumber = (long)[LZPinyinFormatter getNumericValue:[lowerCasePinyinStr characterAtIndex:lowerCasePinyinStr.length -1]];
      int indexOfA = (long)[lowerCasePinyinStr indexOf:charA];
      int indexOfE = (long)[lowerCasePinyinStr indexOf:charE];
      int ouIndex = (long)[lowerCasePinyinStr indexOfString:ouStr];
      if (-1 != indexOfA) {
        ...
        for (int i = (long)[lowerCasePinyinStr length] - 1; i >= 0; i--) {
      ...
      if ((defautlCharValue != unmarkedVowel) && (defautlIndexValue != indexOfUnmarkedVowel)) {
        int rowIndex = (long)[allUnmarkedVowelStr indexOf:unmarkedVowel];

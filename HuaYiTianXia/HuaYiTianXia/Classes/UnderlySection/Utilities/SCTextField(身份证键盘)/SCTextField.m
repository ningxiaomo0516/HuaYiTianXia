...
- (void)setup {
    ...
    [kNotificationCenter addObserver:self selector:@selector(keyboardWillBeginShow:) name:UITextFieldTextDidBeginEditingNotification object:nil];
    [kNotificationCenter addObserver:self selector:@selector(keyboardWillEndShow:) name:UITextFieldTextDidEndEditingNotification object:nil];
}
...
- (void)keyboardWillBeginShow:(NSNotification *)notification {
    ...
    //    TTLog(@"%@ --- %zd", notification, self.willShowKeyboard);
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 8.0) {
        if (![UIView tt_foundViewInView:tempWindow clazzName:@"UIKeyboardAutomatic"]) return;
    }
...
- (void)doneButton:(UIButton *)doneButton{
    // 获得光标所在的位置
    NSUInteger insertIndex = [self tt_selectedRange].location;
    ...
    [string replaceCharactersInRange:self.tt_selectedRange withString:doneButton.currentTitle];
    
    // 重新赋值
    self.text = string;
    
    // 让光标回到插入文字后面
    [self tt_setSelectedRange:NSMakeRange(insertIndex + 1, 0)];
